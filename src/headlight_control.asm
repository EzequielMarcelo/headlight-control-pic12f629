...
;headlight_control.c,41 :: 		if (T0IF_bit) {
	BTFSS      T0IF_bit+0, 2
	GOTO       L_interrupt0
;headlight_control.c,42 :: 		T0IF_bit = 0x00;          // Limpa a flag de interrupção do Timer0
	BCF        T0IF_bit+0, 2
;headlight_control.c,43 :: 		TMR0 = 255 - 4;            // Reinicia o Timer0 para gerar interrupções a cada 1ms (prescaler 1:256)
	MOVLW      251
	MOVWF      TMR0+0
...
;headlight_control.c,55 :: 		INTCON = 0xA0;             // Habilita interrupções globais e do Timer0
	MOVLW      160
	MOVWF      INTCON+0
;headlight_control.c,56 :: 		TMR0 = 255 - 4;            // Configura o Timer0 para gerar interrupção a cada 1 ms (prescaler 1:256)
	MOVLW      251
	MOVWF      TMR0+0
...
;headlight_control.c,58 :: 		WPU |= (1 << 1);           // Ativa pull-up interno em GP1 (UNLOCK_SIGNAL)
	BSF        WPU+0, 1
;headlight_control.c,59 :: 		TRISIO = 0b0001011;       // GP0, GP1 e GP3 como entradas; as demais como saída
	MOVLW      11
	MOVWF      TRISIO+0
...
L__main13:
	...
	GOTO       L_main9
L__main10:
;headlight_control.c,77 :: 		HEADLIGHT = 0;            // Desliga o farol após o delay
	BCF        GP2_bit+0, 2
